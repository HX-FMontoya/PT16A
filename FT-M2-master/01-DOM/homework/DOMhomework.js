...
function buildToDo(todo, index) {
  ...
  <span id={index} className="completeText">barrer</span></div > */
  }
  return toDoShell;
}

const myToDo = new ToDo("barrer");
// ToDo {description: "barrer" complete: false}
...
function buildToDos(toDos) {
  // Tu código acá:
  // ToDos -> [{description: "barrer" complete: false}, {description: "trapear" complete: false}]
  return toDos.map(function (todo, index) {
    return buildToDo(todo, index);
  });
  return toDos.map((todo, index) => buildToDo(todo, index));
  // results -> [<div className="toDoShell">
  // < id=0 className="completeText">barrer<//span></div >, <div className="toDoShell">
  // < id=1 className="completeText">trapear</// span></div >]
}
...
function addToDo() {
  ...
  // {description: "barrer", complete: false}
  toDoItems.push(toDo);
  input.value = "";
  displayToDos();
}
